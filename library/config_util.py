...
def load_user_config(file: str) -> dict:
    ...
    # Auto-fill image_dir for DreamBooth subsets: tasks/<name>.toml → ./tasks/<name>
    _impute_image_dir(config, file)

    return config


def _impute_image_dir(config: dict, config_path: Path) -> None:
    config_dir = config_path.parent
    config_stem = config_path.stem  # "001_topic" (no .toml)
    default_image_dir = os.path.join(".", str(config_dir), config_stem)

    for dataset in config.get("datasets", []):
        for subset in dataset.get("subsets", []):
            # FineTuning subsets have metadata_file; DreamBooth subsets don't.
            # Only auto-fill if image_dir is not explicitly set.
            if "metadata_file" not in subset and "image_dir" not in subset:
                subset["image_dir"] = default_image_dir
                logger.info(f"Auto image_dir: {default_image_dir}")
